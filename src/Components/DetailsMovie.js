...
import {fetchCreditsMovie,fetchMovieDetails} from '../api';
import './details.css'
...
async componentDidMount(){
    const data = await fetchCreditsMovie(this.props.id)
    console.log(data)
    this.setState({ credits:data.data.cast})
    const details = await fetchMovieDetails(this.props.id)
    console.log(details)
...
render()
{
    ...
    <div className="movie-info" style={{
        ...
        <div className="movie-info-content">
            ...
            <h1 className="h1">{details.original_title}</h1>    
